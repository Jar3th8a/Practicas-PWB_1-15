...
class PedidoController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        $pedido = DB::transaction(function () use ($data, $user, &$alertas): Pedido {
        ...
        broadcast(new NuevoPedidoRecibido($pedido->load(['items.producto', 'user'])));

        foreach ($alertas as $alerta) {
            $producto = Producto::find($alerta['producto_id']);

            if ($producto) {
                broadcast(new StockBajoAlerta($producto->refresh(), (int) $alerta['stock_actual']));
            }
        }
